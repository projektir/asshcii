...
namespace SSH.Packets
{
    public abstract class Packet
    {
        ...
        private static int packetHeaderSize = 5;
        ...
        static Packet()
        {
            ...
            foreach (var packet in packets)
            {
                try
                {
                    ...
                    packetTypes[packetInstance.PacketType] = packet;
                }
                catch { }
            }
        }

        public static Packet ReadPacket<TClientState>(Client<TClientState> client, Socket socket)
            where TClientState : ClientState, new()
        {
            if (socket == null)
                return null;
            ...
            byte[] payload = fullPacket.Skip(packetHeaderSize).Take((int)(packetLength - paddingLength - 1)).ToArray();
            ...
            using (ByteReader packetReader = new ByteReader(payload))
            {
                ...
                {
                    Packet packet = Activator.CreateInstance(packetTypes[type]) as Packet;
                ...
                {
                    Console.WriteLine($"Received packet but not implemented: {type}; ignoring");
                }
            }
