...
namespace SSH.Algorithms.Kex
{
        public class DiffieHellmanGroup14SHA1 : IKexAlgorithm
    {
        ...
        // For a subgroup of GF(p); q is the order of the subgroup; S is S's
        // identification string; V_C is C's identification string; S is S's
        // public host key; I_C is C's SSH_MSG_KEXINIT message and S is S's
        ...
        {
            //    p is a large safe prime (RFC 3526)
            P = BigInteger.Parse(MODPGroup2048, NumberStyles.HexNumber);
        ...
        public DiffieHellmanGroup14SHA1()
        {
            ...
            Y = BigInteger.Abs(new BigInteger(bytes));
        }

        public byte[] CreateKeyExchange()
        {
            //  and computes: f = g ^ y mod p.
            BigInteger keyExchange = BigInteger.ModPow(G, Y, P);
        ...
        public byte[] DecryptKeyExchange(byte[] keyEx)
        {
            ...
            byte[] decrypted = BigInteger.ModPow(e, Y, P).ToByteArray();
            if (BitConverter.IsLittleEndian)
